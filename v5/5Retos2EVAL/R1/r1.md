...
1. **Servidor Apache como servidor web principal. Web de la empresa**  
   ...
   - Diseñaran una sencilla página con información del servicio de hosting que ofrecemos. El dominio podría ser terraformadores*____*.freedns.com, por ejemplo, cada uno eligirá el suyo. Accederemos por http://terraformadores*____*.freedns.com 

2. **Seguridad mediante HTTPS**  
   - El sitio web debe funcionar exclusivamente sobre **HTTPS**.  Si en el navegador nos solicitan http://terraformadores.freedns.com deberá mostrarle al usuario https://terraformadores.freedns.com
...
Realizar al menos dos WEBs para dos startups que nos ha solicitado utilizar nuestro alojamiento. 
Estas Web tendran su sitio web alojado en /home/*nombre_startup*/public_html. Crearemos un usuario en el servidor por cada startup. Si la startaup se llama vencedorestic el acceso será mediante https:////terraformadores*____*.freedns.com/~vencedorestic. Para realizar las pruebas hasta que las startsups suban sus webs haz una página sencila de pruebas.

1. **Extras de configuración**
   - Añadir un directorio con segurida digest en la web corporativa en el cual almacenaremos páginas con información sensible de la administración. El directorio debe llamarse "/admin". Accederiasmos con https:////terraformadores*____*.freedns.com/admin/
   ...
   ```sh 
      ...
      ServerTokens Prod
   ```
   - Configuración de PHP (php-fpm + opcache) y mpm.
...
1. **Crear un nuevo sitio web en nuestro servidor para un nuevo cliente**  
   ...
   - El sitio con nombre similar a `https://www.ecoamazonia*____*.freedens.com`  debe estar en `/srv/www/ecoamazonia/`.
   ...
   Nos pide una EC2 dedicada para él con ip publica fija. La web se llamará algo similar a www.ticpremiummaxpowerxxxxx.freedns.com
   Además:
